...
int Snake::gateDirection(Vector gate, int map[40][40]){
...
void Snake::moveSnakeHead(int map[40][40]){
	...
	for(int i=0; i<wall.size(); i++) { 
		if(snake_vec[0] == wall[i]) { 
			if(snake_vec[0] == gate[1]) { 
				...
				setEnd(true);
				removeGate(map);
				whyDead = "notice : You can't go through the wall!"; 
			}
		}
	}
}
...
char* Snake::setMaptoList(int map[40][40]){ 
...
void Snake::setGate(int map[40][40]) { 
...
void Snake::removeGate(int map[40][40])
{
